...
export const storeRoom = (location, details, images, updatedRoom, deleteImages, addedImages, userId) => {
  if (location.lng || location.lat || details.price || details.title || details.description || images.length) {
    localStorage.setItem(userId, JSON.stringify({ location, details, images, updatedRoom, deleteImages, addedImages, }))
    return true
  } else {
    return false
  }
...
export const getUserRooms = async (currentUser) => {
  const result = await fetchData({ url: `${url}/user/${currentUser.id}`, method: 'GET', token: currentUser?.token })
